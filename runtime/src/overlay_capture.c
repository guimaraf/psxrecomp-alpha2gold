 ...
 * The JSON output uses proven dynamic-code regions (DMA dirty plus exact
 * CPU-written text divergence), not individual DMA blocks.
 * -------------------------------------------------------------------------*/
...
void overlay_capture_write_json(void)
{
    ...
    if (!bitmap) return;
    /* The DMA dirty bitmap is not the whole dynamic-code set: games can unpack
     * code with ordinary CPU stores over their boot EXE. Exact static-range
     * validation records those proven mismatches in the text-divergence bitmap.
     * Union both evidence streams so execution-proven CPU-written code reaches
     * the same content-validated overlay compiler/cache path as DMA overlays. */
