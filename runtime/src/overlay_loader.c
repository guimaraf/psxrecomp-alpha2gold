...
typedef struct {
    ...
    uint64_t  native_entries;            /* live native dispatches at entry    */
    uint64_t  native_continuations;      /* live native CPS continuation calls */
...
static void try_sljit_region(uint32_t addr) {
    ...
    if (!dirty_ram_code_is_dynamic(phys)) return; /* only JIT real runtime code */
...
static void init_callbacks(void) {
    ...
    /* ABI v12: an overlay-resident RFE updates the same exception-escape
     * state used by recompiled BIOS/game code in the main runtime. */
    s_callbacks.psx_rfe_mark_escape = psx_rfe_mark_escape;
}
...
retry_candidates:
    ...
    if (head < 0 && s_active && overlay_cache_window_contains(phys)) {
        ...
        /* An exact manifested entry is a stronger dispatch contract than an
         * enclosing CPS range owner. A previously-loaded root-only bundle may
         * own this PC as a continuation while a later complete bundle exports
         * the exact entry. Always publish that exact live bundle once; ordinary
         * interior continuations still have lazy_exact == 0 and retain the
         * range-owner fast path. */
        ...
        if (head < 0 && g_sljit_tier_enabled && (s_diff_mode || s_sljit_live) && !s_in_shadow &&
            ...
            if (g_sljit_async) {
                ...
                if (!sljit_already_tried(phys)) {
                    uint8_t *ram = memory_get_ram_ptr();
                    if (ram && dirty_ram_code_is_dynamic(phys)) {
                        /* only JIT runtime code (DMA- or CPU-installed) */
                        sljit_mark_tried(phys);
...
try_cps_range:
     ...
     *
     * A historical exact entry may also occupy this PC. Exact live entries keep
     * priority through the main chain below, but once every exact candidate is
     * stale/invalid, retry the range owner. Otherwise a stale interior shard can
     * mask the current function's valid continuation forever. */
    if ((head < 0 || exact_chain_exhausted) && g_psx_cps_mode) {
...
#endif
                if (!s_in_shadow) c->native_continuations++;
    ...
    /* A post-exact CPS retry must not walk the already-exhausted exact chain a
     * second time. Continue directly to lazy publication / interpreter fallback
     * if no live range owner exists. */
    if (exact_chain_exhausted) goto after_exact_candidates;
    ...
    for (int i = head; i >= 0; i = s_cand[i].next) {
        ...
        if (matched) {
            ...
            s_native_inprogress = c->addr;
#endif
            if (!s_in_shadow) c->native_entries++;
            s_native_calls_total++;
    ...
    /* Exact candidates exist at this PC, but none matched live RAM. This may be
     * a legitimate CPS continuation inside another current candidate's range.
     * Give that range owner one chance before publishing more cache history or
     * falling back to the interpreter. overlay_find_by_range() validates every
     * variant and skips known-stale owners. */
    if (head >= 0 && g_psx_cps_mode) {
        exact_chain_exhausted = 1;
        goto try_cps_range;
    }

after_exact_candidates:
...
int overlay_loader_dump_candidates(char *out, int cap) {
    int n = 0;
    n += snprintf(out + n, cap - n, "[");
    for (int i = 0; i < s_cand_n && n < cap - 240; i++) {
        Candidate *c = &s_cand[i];
        uint32_t live = cand_crc(c);
        ...
        n += snprintf(out + n, cap - n,
            ...
            "\"native_entries\":%llu,\"native_continuations\":%llu}",
            i ? "," : "", c->addr, c->state, c->nranges,
            c->crc_code, live, (live == c->crc_code) ? 1 : 0,
            c->val_gen, sum, c->dll, c->diff_passes,
            (unsigned long long)c->native_entries,
            (unsigned long long)c->native_continuations);
    }
    n += snprintf(out + n, cap - n, "]");
    return n;
}
...
int overlay_loader_dump_candidates_at(uint32_t addr, char *out, int cap) {
    ...
    int first = 1;
    n += snprintf(out + n, cap - n, "[");
    for (int i = 0; i < s_cand_n && n < cap - 260; i++) {
        Candidate *c = &s_cand[i];
        ...
        n += snprintf(out + n, cap - n,
            ...
            c->val_gen, sum, c->dll, c->diff_passes,
            (unsigned long long)c->native_entries,
            (unsigned long long)c->native_continuations, c->device_touch);
        first = 0;
    }
    n += snprintf(out + n, cap - n, "]");
    return n;
