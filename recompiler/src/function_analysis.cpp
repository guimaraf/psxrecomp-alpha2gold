...
struct ExactWalkResult {
    ...
    std::set<uint32_t> reachable_direct_jal_targets;
...
FunctionAnalysisResult FunctionAnalyzer::analyze_exact_entries(const std::vector<uint32_t>& entries) {
    ...
    auto walk = [&](uint32_t entry, uint32_t hard_cap) {
        ...
        while (!work.empty()) {
            ...
            case ExactCfKind::Jal:
                ...
                if (in_function(pc + 8u)) work.push(pc + 8u);
                if (in_exe(cf.target)) {
                    /* A reachable jal is definitive code-target evidence even
                     * when the callee is a leaf/thunk without a stack
                     * prologue. Keep the conservative boundary gate for this
                     * analyzer's own roots; main_psx classifies the complete
                     * evidence set against final range ownership. */
                    wr.reachable_direct_jal_targets.insert(cf.target);
                    if (callable_direct_jal_target(cf.target)) {
    ...
    for (size_t i = 0; i < starts_vec.size(); i++) {
        ...
        reachable_direct_jal_targets.insert(
            wr.reachable_direct_jal_targets.begin(),
            wr.reachable_direct_jal_targets.end());
    ...
    result.reachable_direct_jal_targets.assign(
        reachable_direct_jal_targets.begin(),
        reachable_direct_jal_targets.end());
