...
using psx_launcher::deadzone_pct_to_raw;
using psx_launcher::deadzone_raw_to_pct;
using psx_launcher::kDigitalDeadzonePct;
...
struct LauncherModel {
    ...
    bool bios_hle        = true;  // skip BIOS boot (HLE BIOS boot shell-skip); default on
    bool spu_hq          = false;
...
// Digital mode folds the left stick onto the D-pad. Reset its threshold when
// the user explicitly selects Digital so an old zero/small analog deadzone
// cannot turn harmless centre drift into a held direction. A later manual
// deadzone change remains untouched until Digital is selected again.
bool select_player_pad_mode(LauncherModel& m, int player, int mode) {
    (player == 0 ? m.p1_mode : m.p2_mode) = mode;
    if (mode != PSXRecompV4::PAD_MODE_DIGITAL) return false;
    m.deadzone_pct = kDigitalDeadzonePct;
    return true;
}
...
struct DeviceOption {
    ...
    int         ordinal; // 0, 1, 2... for devices sharing the same GUID
...
int find_or_add_device_index(std::vector<DeviceOption>& opts, const std::string& dev) {
    ...
    // Exact match: GUID and ordinal
    for (size_t i = 0; i < opts.size(); i++) {
        if (opts[i].kind == 2 && opts[i].guid == target_guid && opts[i].ordinal == target_ord)
            return (int)i;
    }
    // Fallback: first available controller matching GUID (backward compatibility)
    for (size_t i = 0; i < opts.size(); i++) {
...
void refresh_labels(LauncherModel& m) {
    ...
    m.interpolation_target_visible = false;
           ...
           const GameInfo& game, const char* assets_dir)
{
    ...
    m.deadzone_pct = io.has_deadzone ? deadzone_raw_to_pct(io.deadzone)
                                      : kDigitalDeadzonePct;
    if (io.has_p1_device) {
    ...
    c.BindEventCallback("set_mode_p1",
        [&m, handle, dev_opts, dirty_player](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList& args) mutable {
            ...
            refresh_player(m, 0, dev_opts); dirty_player(0);
    ...
    c.BindEventCallback("set_mode_p2",
        [&m, handle, dev_opts, dirty_player](Rml::DataModelHandle, Rml::Event&, const Rml::VariantList& args) mutable {
            ...
            refresh_player(m, 1, dev_opts); dirty_player(1);
            if (deadzone_reset) handle.DirtyVariable("deadzone_pct");
        });
    ...
    if (result == Result::Launch) {
        ...
        io.deadzone = deadzone_pct_to_raw(m.deadzone_pct); io.has_deadzone = true;
